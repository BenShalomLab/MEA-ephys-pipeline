...
if __name__ == "__main__":
    ...
    for postprocessed_folder in postprocessed_folders:
        ...
        # NEW CODE:
        logging.info("Skipping noise-neural classifier - using basic QC only")
        noise_units = pd.DataFrame()  # No noise units - keep all QC-passing units
        ...
        # NEW CODE:
        logging.info("Skipping SUA/MUA classifier - labeling all QC-passing units as SUA")
        unit_ids = analyzer.unit_ids
        all_labels = pd.DataFrame({
            'prediction': ['sua'] * len(unit_ids),
            'probability': [1.0] * len(unit_ids)
        }, index=unit_ids)
